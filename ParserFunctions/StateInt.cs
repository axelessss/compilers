namespace Parse
{
    ...
    {
        private void StateInt(string input, ref int position) //5
        {
            ...
            // Проверяем, начинается ли ключевое слово "int" с текущей позиции
            foreach (char c in expectedKeyword)
            {
                ParserError error = new ParserError("Ожидалось ключевое слово \"int\"", keywordStartPos + 1, position + 1);
                while (true)
                {
                    if (position >= input.Length || char.IsWhiteSpace(input[position]) || input[position] == ')' || input[position] == '\n' || input[position] == '{')
                    {
